...
int main() {
    printf("Advent of Code Day 2 - part2\n");
    ...
    while(line != NULL)
    {
        ...
        if(check_arr_validity(intarr, size)) total++;
            ...
            {
                int *temparr = (int *)malloc((size - 1) * sizeof(int));
                // skip the index equal to i and put all rest in temp
                int k = 0;
                ...
                if(check_arr_validity(temparr, size-1))
                {
                    total++;
                    free(temparr);
                    break;
